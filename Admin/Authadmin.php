<?php
    ...
    $check_admins = mysqli_query($dp, "SELECT * FROM `персонал` WHERE `Логин` = '$login' AND `Пароль` = '$password'");
    if (mysqli_num_rows($check_admins) > 0) {

        $admin = mysqli_fetch_assoc($check_admins);

        $_SESSION['ADMIN'] = [
            "id" => $admin['ID сотрудника'],
            "full_name" => $admin['ФИО'],
            "email" => $admin['E-mail'],
            "tel" => $admin['Номер телефона'],
            ...
            <div class="container">
                <div class="navbar-nav">
                    <div class="navbar-brand">
                        <a href="Authadmin.php"><img src="../logo1.png" alt="Логотип"></a><a>
                    </div>
                </div>
            </div>
        </div>
